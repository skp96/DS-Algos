...
class MinMaxStack {
  ...
  push(val) {
    ...
    if (!this.minValStorage.length || newNode.value <= this.minValStorage[this.minValStorage.length - 1].value) {
      this.minValStorage.push(newNode);
    }

    if (!this.maxValStorage.length || newNode.value >= this.maxValStorage[this.maxValStorage.length - 1].value) {
      this.maxValStorage.push(newNode);
    }
  ...
  pop() {
    ...
    if (this.top.value === this.maxValStorage[this.maxValStorage.length - 1].value) {
      this.maxValStorage.pop();
    }

    if (this.top.value === this.minValStorage[this.minValStorage.length - 1].value) {
  ...
  min() {
    return this.minValStorage[this.minValStorage.length - 1] || null;
  }

  max() {
    return this.maxValStorage[this.maxValStorage.length - 1] || null;
  }
}
